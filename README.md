...
目前版本以 Chrome Side Panel 為主介面，固定在瀏覽器右側，頁面導航時保持開啟，包含 X ETL、長文整理、自訂流程、Prompt Manager、Schema 與設定共六個 Tab。

## 目前狀態

- **Side Panel 遷移完成（2026-05-03）**：主介面改為 `sidepanel.html`；`popup.html` 保留作為開發參考。
- **Distill Tab 重構完成（2026-05-03）**：內部邏輯改以 5 個 Block 物件封裝，並抽檔至 `src/blocks/*.js`。
...
├── status.md
└── src/
    ├── popup.js             ← 主要工作流邏輯（TODO: 未來重命名為 sidepanel.js）
    ├── popup-ui-patch.js
    ├── background.js
    ├── cs_grok.js
    ├── cs_ai.js
    └── blocks/              ← Distill 5 個 Block 物件（plain script，popup.js 之前載入）
...
- 與 Distill Tab 共用 Prompt 庫與 Schema 庫，但彼此狀態完全獨立。
...
- 主要流程放在 `src/popup.js`。
- Distill Block 物件放在 `src/blocks/*.js`，以 plain `<script>` 在 popup.js 之前載入。
